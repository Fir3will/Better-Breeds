...
public class MPUtil
{
	...
	{
		String val = "";
		final String obs = String.valueOf(EnumChatFormatting.OBFUSCATED);
	...
	{
		final World worldObj = target.worldObj;
		final EntityArrow entityarrow = new EntityArrow(worldObj, entity, target, 1.6F, 14);
		final int i = EnchantmentHelper.getEnchantmentLevel(Enchantment.power.effectId, stack);
		final int j = EnchantmentHelper.getEnchantmentLevel(Enchantment.punch.effectId, stack);
	...
	{
		final ArrayList<TileEntity> tiles = JavaHelp.newArrayList();
	...
	{
		final ArrayList<Block> blocks = JavaHelp.newArrayList();
	...
	public static void dropItemAsEntity(World world, double x, double y, double z, boolean useClone, ItemStack stack)
	{
		...
		final ItemStack item = useClone ? stack.copy() : stack;
		if (MPUtil.isServerSide() && item != null && item.stackSize > 0)
		{
			...
			final EntityItem entityItem = new EntityItem(world, x + rx, y + ry, z + rz, item.copy());
	...
	{
		final int random = Rand.nextInt(Item.itemRegistry.getKeys().size());
	...
	{
		final Item item = getRandomItem();
		if (item.getClass().getName().startsWith("main.betterbreeds.")) return item;
	...
	{
		for (final EntityPlayer player : getServerPlayers())
		{
