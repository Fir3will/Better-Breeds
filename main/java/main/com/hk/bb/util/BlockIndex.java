...
public class BlockIndex
{
	...
	public float getBreakSpeed(ItemStack stack)
	{
		...
		if (f > 1.0F)
		{
			final int i = EnchantmentHelper.getEnchantmentLevel(Enchantment.efficiency.effectId, stack);

			if (i > 0)
			{
				final float f1 = i * i + 1;

				final boolean canHarvest = ForgeHooks.canToolHarvestBlock(block, metadata, stack);
