...
public class RecipeUtils
{
	...
	public static ItemStack[] getFirstRecipeForItem(ItemStack resultingItem)
	{
		...
		for (final IRecipe recipe : getVanillaRecipes())
		{
			if (recipe == null)
			{
				continue;
			...
			final Object[] input = getRecipeInput(recipe);
			if (input != null)
			{
	...
	public static ItemStack[] getFirstRecipeForItem(Item item)
	{
		for (int i = 0; i < item.getItemStackLimit(new ItemStack(item)); i++)
		{
			...
			{
				final ItemStack[] stacks = getFirstRecipeForItem(new ItemStack(item, i, j));
	...
	private static Object[] getShapedOreRecipe(ShapedOreRecipe recipe)
	{
		...
		int inputIndex = 0;
		final Object[] grid = new Object[9];
		for (int y = 0; y < 3; y++)
		{
			for (int x = 0; x < 3; x++)
			{
