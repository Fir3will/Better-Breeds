...
public final class Vector3I implements Cloneable, java.io.Serializable
{
	...
	{
		final int x1 = (int) (x * Math.cos(Math.toRadians(degrees)) - z * Math.sin(Math.toRadians(degrees)));
		final int z1 = (int) (x * Math.sin(Math.toRadians(degrees)) + z * Math.cos(Math.toRadians(degrees)));
	...
	{
		final int x1 = (int) (x * Math.cos(degrees) - y * Math.sin(degrees));
		final int y1 = (int) (x * Math.sin(degrees) + y * Math.cos(degrees));
		x = x1;
		y = y1;
		return this;
	}
	...
	public Vector3I cross(int otherX, int otherY, int otherZ, Vector3I result)
	{
		...
		final int resX = y * otherZ - z * otherY;
		final int resY = z * otherX - x * otherZ;
		final int resZ = x * otherY - y * otherX;
	...
	{
		final int tempx = y * otherZ - z * otherY;
		final int tempy = z * otherX - x * otherZ;
		z = x * otherY - y * otherX;
		x = tempx;
		y = tempy;
		return this;
	...
	{
		final int n = dot(other); // A . B
		final int d = other.lengthSquared(); // |B|^2
	...
	{
		final int dotProduct = dot(otherVector);
		final int angle = (int) Math.acos(dotProduct);
		return angle;
	}
