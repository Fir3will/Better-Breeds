...
public final class Vector3F implements Cloneable, java.io.Serializable
{
	...
	{
		final float x1 = (float) (x * Math.cos(Math.toRadians(degrees)) - z * Math.sin(Math.toRadians(degrees)));
		final float z1 = (float) (x * Math.sin(Math.toRadians(degrees)) + z * Math.cos(Math.toRadians(degrees)));
	...
	{
		final float x1 = (float) (x * Math.cos(degrees) - y * Math.sin(degrees));
		final float y1 = (float) (x * Math.sin(degrees) + y * Math.cos(degrees));
		x = x1;
		y = y1;
		return this;
	}
