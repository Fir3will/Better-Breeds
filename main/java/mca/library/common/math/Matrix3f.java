...
public final class Matrix3f implements Cloneable, java.io.Serializable
{
	...
	public void get(float[] data, boolean rowMajor)
	{
		...
		else throw new IndexOutOfBoundsException("Array size must be 9 or 16 in Matrix3f.get().");
	}
	...
	{
		if (matrix.length != 3 || matrix[0].length != 3) throw new IllegalArgumentException("Array must be of size 9.");
	...
	{
		if (matrix.length != 9) throw new IllegalArgumentException("Array must be of size 9.");
	...
	public Matrix3f invert(Matrix3f store)
	{
		...
		if (FastMath.abs(det) <= FastMath.FLT_EPSILON) return store.zero();
	...
	{
		final float det = determinant();
		if (FastMath.abs(det) <= FastMath.FLT_EPSILON) return zero();
	...
	{
		if (!(o instanceof Matrix3f) || o == null) return false;
